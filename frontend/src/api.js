...
export const api = {
  ...
  // BTC 5M Micro-Test Mode (opt-in, isolated, minimum-size live test)
  btc5mMicroTestStatus: () => request('/api/btc5m/micro-test/status'),
  btc5mMicroTestRunOnce: (place = false) => request(`/api/btc5m/micro-test/run-once?place=${place}`, { method: 'POST' }),
  ...
  // BTC 5M Alpha Research Platform (fair-value / ensemble / nightly; paper only)
  btc5mResearchStatus: () => request('/api/btc5m/research/status'),
  btc5mResearchRun: (build = false, limitMarkets = 60) => request(`/api/btc5m/research/run?build=${build}&limit_markets=${limitMarkets}`, { method: 'POST' }),
  ...
  btc5mLiveMakerResetLock: () => request('/api/btc5m/live-maker/reset-lock', { method: 'POST' }),
